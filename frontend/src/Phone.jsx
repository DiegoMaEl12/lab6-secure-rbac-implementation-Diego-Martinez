...
function Phone({ id, brand, model, price, storage, os, quantity, onDelete, onUpdate, onAddToCart }) {
    const { isAdmin } = useAuth();
    const [isEditing, setIsEditing] = useState(false);
    const [tempData, setTempData] = useState({ brand, model, price, storage, os, quantity });
    ...
    return (
        <div className="book-row">
            <div className="book-info">
                <h3>{brand} {model}</h3>
                <p><strong>Operating System:</strong> {os} | <strong>Storage:</strong> {storage}GB | <strong>Price:</strong> ${Number(price).toFixed(2)}</p>
            </div>
